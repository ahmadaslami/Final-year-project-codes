...
def delete_data_by_id():
    ...
    user_input = entry.get()
    try:
        result = collection.find_one_and_delete({"citizen_id": user_input})
        if result:
            result_label1.config(text="Document deleted successfully.")
        else:
            result_label1.config(text="Document not found.")
    except Exception as e:
        result_label1.config(text=f"Error: {e}")
...
button_search = Button(window, text="Search by ID", command=fetch_data_by_id, font=('Arial', 16),
                    relief=tk.RAISED, fg="white", bg="skyblue")
button_search.grid(row=0, column=2, sticky='w')

button_delete = Button(window, text="Delete by ID", command=delete_data_by_id, font=('Arial', 16),
                    relief=tk.RAISED, fg="white", bg="red")
button_delete.grid(row=0, column=3, sticky='w')
