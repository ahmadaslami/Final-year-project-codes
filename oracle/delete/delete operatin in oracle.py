...
def fetch_citizen_data():
    ...
    user_input_id = str(id_entry.get())
...
def delete_citizen_data():
    try:
        ...
        # Define the SQL query to delete a citizen record
        delete_query = """
        DELETE FROM citizen
        WHERE citizen_id = :user_input_id
        """

        cursor = connection.cursor()
        cursor.execute(delete_query, user_input_id=user_input_id)
        ...
        result_label.config(text="Citizen record deleted successfully.")
    except cx_Oracle.Error as error:
        result_label.config(text=f"Database Error: {error}")
...
delete_button = Button(window, text="Delete Record", command=delete_citizen_data, font=("Helvetica", 10), bg="red",
