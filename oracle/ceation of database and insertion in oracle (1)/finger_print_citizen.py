...
def insert_photos_from_folder(folder_path,num_rows):
    ...
    for finger_print_citizen_id in range(1,num_rows):
        random_photo = random.choice(photo_files)
        with open(os.path.join(folder_path, random_photo), 'rb') as photo_file:
            photo_data = photo_file.read()
            # Generate a random ID
            cursor.execute(
                "INSERT INTO finger_print_citizen (finger_print_citizen_id, photo) VALUES (:id, :photo_data)",
                id=finger_print_citizen_id, photo_data=photo_data)
