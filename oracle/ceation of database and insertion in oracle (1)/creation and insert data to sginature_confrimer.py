...
position = """ CREATE TABLE signature_confirmer (
        signature_confirmer_id   NUMBER PRIMARY KEY, 
        photo BLOB)"""
...
def insert_photos_from_folder(folder_path, num_rows):
    ...
    for finger_print_confirmer_id in range(num_rows):
        ...
        with open(os.path.join(folder_path, random_photo), 'rb') as photo_file:
            ...
            cursor.execute(
                "INSERT INTO signature_confirmer (signature_confirmer_id, photo) VALUES (:id, :photo_data)",
                id=finger_print_confirmer_id, photo_data=photo_data)


your_folder_path = "E:\Every thing\Mobile Applications\image3"
